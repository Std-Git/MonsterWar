...
private:
    ...
    // 直接切换场景
    void pushScene(std::unique_ptr<Scene>&& scene);                 ///< @brief 压入一个新场景, 使其成为活动场景
    void popScene();                                                ///< @brief 弹出栈顶场景
    void replaceScene(std::unique_ptr<Scene>&& scene);              ///< @brief 清理场景栈所有肠场景，将此场景设为栈顶场景
