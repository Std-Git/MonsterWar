...
void LevelLoader::loadImageLayer(const nlohmann::json& layer_json, Scene& scene)
{
    ...
    // 获取图像图层的偏移量 (json 中没有则代表未设置，给默认值即可)
...
void LevelLoader::loadObjectLayer(const nlohmann::json& layer_json, Scene& scene)
{
    ...
    for (const auto& object : objects)
    {
        ...
        if (gid == 0)       // 如果 gid == 0 (即不存在)，则代表自己绘制的形状(如碰撞盒、触发器等，未来按需处理)
        {
            // 非矩形对象会有额外标识(目前不考虑)
            ...
            else
            {
                ...
                auto rotation = object.value("rotation", 0.0f);
                    // 添加 TransformComponent, 缩放默认为 1.0f
                ...
                auto* cc = game_object->addComponent<engine::component::ColliderComponent>(std::move(collider));
                    // 自定义形状通常是 trigger 类型, 除非显示指定 (因此默认为真)
        ...
        else        // 如果 gid 存在，则按照图片解析流程
        {
            // 根据 gid 获取必要信息，每个 gid 对应一个游戏对象
            auto tile_info = getTileInfoByGid(gid);
            if (tile_info.sprite.getTextureId().empty())
            {
                spdlog::error("gid为 {} 的对象没有图片纹理", gid);
                continue;
            }
            // 获取 Transform 相关信息
            auto position = glm::vec2(object.value("x", 0.0f), object.value("y", 0.0f));
            auto dst_size = glm::vec2(object.value("width", 0.0f), object.value("height", 0.0f));
            position = glm::vec2(position.x, position.y - dst_size.y);   // y 轴坐标需要调整 (左下角到左上角, Tiled 中对象是以左下角为坐标点)

            auto rotation = object.value("rotation", 0.0f);
            auto src_size_opt = tile_info.sprite.getSourceRect();
            if (!src_size_opt)      // 正常情况下，所有对象的 Sprite 都设置了源矩形，没有代表某处出错
            {
                spdlog::error("gid为 {} 的对象没有源矩形", gid);
                continue;
            }
            ...
            // 获取瓦片 json 信息   1. 必然存在，因为 getTileInfoByGid(gid) 函数已经顺利执行
            //                     2. 这里再获取 json 实际上检索了两次，未来可以优化
            auto tile_json = getTileJsonByGid(gid);

            // 获取碰撞器信息：如果是 SOLID 类型，则添加物理组件，且图片原举行大小就是碰撞盒大小
...
void LevelLoader::addAnimation(const nlohmann::json &anim_json, engine::component::AnimationComponent *ac, const glm::vec2 &sprite_size)
{
    ...
    for (const auto& anim : anim_json.items())
    {
        ...
        // 获取可能存在的动画信息
        auto duration_ms = anim_info.value("duration", 100);        // 默认帧间隔为 100 ms
        ...
        // 将 Animation 添加到 AnimationComponent 中
...
void LevelLoader::addSound(const nlohmann::json& sound_json, engine::component::AudioComponent* audio_component)
{
    ...
    {
        spdlog::error("无效的音频 JSON 或 AudioComponent 指针");
        return;
    }
    // 遍历音效 json 对象中的每个键值对 (音效 id : 音效路径)
    for (const auto& sound : sound_json.items())
    {
        std::string_view sound_id = sound.key();
        std::string_view sound_path = sound.value();
    ...
    if (tile_json.contains("properties"))
    {
        ...
        for (auto& property : properties)
        {
            ...
            else if (property.contains("name") && property["name"] == "slope")
            {
                ...
                if (slope_type == "0_1") { return engine::component::TileType::SLOPE_0_1; }
                else if (slope_type == "1_0") { return engine::component::TileType::SLOPE_1_0; }
                else if (slope_type == "0_2") { return engine::component::TileType::SLOPE_0_2; }
                else if (slope_type == "2_0") { return engine::component::TileType::SLOPE_2_0; }
                else if (slope_type == "2_1") { return engine::component::TileType::SLOPE_2_1; }
                else if (slope_type == "1_2") { return engine::component::TileType::SLOPE_1_2; }
                else
                {
                    spdlog::error("未知斜坡类型: {}", slope_type);
...
engine::component::TileInfo LevelLoader::getTileInfoByGid(int gid)
{
    ...
    --tileset_it;       // 前移一个元素，这样就得到不大于 gid 的最近一个元素 (需要的)

    const auto& tileset = tileset_it->second;
    auto local_id = gid - tileset_it->first;    // 计算瓦片在图块集中的局部 ID
    ...
    if (tileset.contains("image"))      // 这是单一图片的情况
    {
        ...
        auto tile_type = getTileTypeById(tileset, local_id);    // 获取瓦片类型，(只有瓦片id,还没有找具体瓦片 json)
        return engine::component::TileInfo(sprite, tile_type);
    }
    else        // 多图片的情况
    {
        if (!tileset.contains("tiles"))     // 没有 tiles 字段的话不符合数据格式要求，直接返回空的瓦片信息
        {
        ...
        for (const auto& tile_json : tiles_json)
        {
            ...
            if (tile_id == local_id)    // 找到对应的瓦片，进行后续操作
            {
                ...
                {
                    spdlog::error("Tileset 文件 '{}' 中 id 为 {} 的瓦片缺少 'image' 属性", tileset_it->first, tile_id);
                    return engine::component::TileInfo();
                }
                ...
                // 从 json 中获取源矩形信息
                SDL_FRect texture_rect =        // tiled 中源矩形只有设置了才会有值，没有就是默认值
                {
                ...
                engine::render::Image sprite{ texture_id, texture_rect };
                auto tile_type = getTileType(tile_json);        // 获取瓦片类型 (已经有具体瓦片 json了)
                return engine::component::TileInfo(sprite, tile_type);
            }
        }
    }
    // 如果运行到了这里，说明查找失败，返回空的瓦片信息
    spdlog::error("图块集 '{}' 中未找到 gid 为 {} 的瓦片", tileset_it->first, gid);
    return engine::component::TileInfo();
}

std::optional<nlohmann::json> LevelLoader::getTileJsonByGid(int gid) const
{
    // 1. 查找 tileset_data_ 中第一个键小于等于gid 的最近元素
    auto tileset_it = tileset_data_.upper_bound(gid);
    if (tileset_it == tileset_data_.begin())
    {
        spdlog::error("gid为 {} 的瓦片未找到图块集", gid);
    ...
    // 3. 遍历 tiles 数组，根据 id 查找对应的瓦片并返回瓦片 json
    const auto& tiles_json = tileset["tiles"];
    for (const auto& tile_json : tiles_json)
    {
        auto tile_id = tile_json.value("id", 0);
        if (tile_id == local_id)    // 找到对应的瓦片，返回瓦片 json
        {
            return tile_json;
        }
    }
