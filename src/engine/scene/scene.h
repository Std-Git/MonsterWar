...
public:
    ...
    virtual ~Scene();               // 1. 基类必须声明虚析构函数才能让派生类析构函数被正确调用
                                    // 2. 析构函数定义必须写在 cpp 中, 不然需要引入 GameObject 头文件
    // 禁止拷贝和移动构造函数
    ...
    void setInitialized(bool is_initialized) { is_initialized_ = is_initialized; }      ///< @brief 设置场景是否已初始化
