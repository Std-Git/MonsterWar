...
TextureManager::TextureManager(SDL_Renderer* renderer) : renderer_(renderer)
{
    if (!renderer_)
    {
        // 关键错误，无法继续，抛出异常，将由catch语句获取并进行处理
        throw std::runtime_error("TextureManager 构造失败：渲染器指针为空");
    }
    // SDL3 中无需手动 Init SDL_Image
...
{ 
    ...
    // 如果未找到，判断是否提供了 file_path
    if (file_path.empty())
    {
        spdlog::error("纹理 '{}' 未找到缓存，且未提供纹理路径，返回 nullptr", id);
        return nullptr;
    }
...
{ 
    ...
    {
        spdlog::error("无法获取纹理尺寸: {}", file_path);
        return glm::vec2(0, 0);
    }
...
void TextureManager::unloadTexture(entt::id_type id)
{
    ...
    if (it != textures_.end())
    {
        ...
        textures_.erase(it); // unique_ptr 会自动释放资源
    }
    else
    {
...
void TextureManager::clearTextures()
{
    if (!textures_.empty())
    {
        ...
        textures_.clear(); // unique_ptr 会自动释放资源
    }
}
