...
{
/**
 * @brief 管理 SDL_Mixer 音效和音乐 Mix_Chunk / Mix_Music
 * 
 * 提供音频资源的加载和缓存功能，构造失败会抛出异常
 * 仅供 ResourceManager 使用
 */
class AudioManager
{
...
public:
     ...
     * @throws std::runtime_error SDL_Mixer 初始化失败或打开音频设备失败
     */
    AudioManager();

    ~AudioManager();            ///< @brief 手动添加析构函数，清理资源并关闭 SDL_Mixer;
