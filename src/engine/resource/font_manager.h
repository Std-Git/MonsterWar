...
{
// 定义字体键类型 (路径 + 大小)
using FontKey = std::pair<entt::id_type, int>;        // std::pair 是标准库中的一个类模板，用于将两个值组合成一个单元

// FontKey 的自定义哈希函数 (std::pair<std::string, int>), 用于 std::unorderer_map
struct FontKeyHash
{
 ...
 * 
 * 提供字体的加载和缓存功能，通过文件路径和文字大小获取
 * 构造失败会抛出异常，仅供 ResourcesManager 使用
 */
class FontManager
{
...
private:
    ...
    // unordered_map 的键需要能换为哈希值，对于基础数据库类型，系统会自动转换
    // 但是对于自定义类型 (系统无法自动转换) ,则需要提供自定义哈希函数(第三个模板函数)
    std::unordered_map<FontKey, std::unique_ptr<TTF_Font, SDLFontDeleter>, FontKeyHash> fonts_;

public:
    /**
     * @brief 构造函数，初始化 SDL_ttf
     * @throws std::runtime_error SDL_ttf 初始化失败
     */
    FontManager();
