...
namespace engine::input
{
    ...
    public:
        ...
        // 动作状态检查,提供不同的使用选择
        bool isActionDown(entt::id_type action_name_id) const;     ///< @brief 检查动作是否被按下 (持续按下或本帧按下)
        bool isActionPressed(entt::id_type action_name_id) const;  ///< @brief 检查动作是否在本帧刚刚按下
        bool isActionReleased(entt::id_type action_name_id) const; ///< @brief 检查动作是否在本帧刚刚释放
