...
namespace engine::input
{
    ...
    private:
        ...
        entt::dispatcher* dispatcher_; ///< @brief 用于分发动作事件的事件分发器

        /**
         * @brief 核心数据结构：存储动作名称函数列表的映射
         *
         * @note 每个动作有三个状态： PRESSED, HELD, RELEASED,每个状态对应一个回调函数
         * @note 绑定动作时再插入元素(懒加载),初始化为空
         */
    ...
    public:
        ...
        // 动作状态检查,提供不同的使用选择
        bool isActionDown(std::string_view action_name) const;     ///< @brief 检查动作是否被按下 (持续按下或本帧按下)
        bool isActionPressed(std::string_view action_name) const;  ///< @brief 检查动作是否在本帧刚刚按下
        bool isActionReleased(std::string_view action_name) const; ///< @brief 检查动作是否在本帧刚刚释放
