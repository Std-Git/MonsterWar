...
private:
    ...
    bool need_remove_ = false;  ///< @brief 延迟删除的表示，将来有场景类负责删除
...
public:
    ...
    T* addComponent(Args&&... args)
    {
        // 检测组件是否合法     /* static_assert(condition, message)：静态断言，在编译期检测，无任何性能影响 */
                                /* std::is_base_of<Base, Dervied>::value -- 判断 Base 类型是否是 Derived 类型的基类*/
        static_assert(std::is_base_of<engine::component::Component, T>::value, "T 必须继承自 Component");
        // 获取类型标识         /* typeid(T) -- 用于获取一个表达式或类型的运行信息 (RTTI), 返回 std::type_info& */
                                /* std::type_index -- 针对 std::type_info 对象的包装器，主要设计用来关联容器 (如 std::map) 的键 */
        ...
        ptr->init();                                         // 初始化组件  (因此必须用 ptr 而不能用 new_component)
    ...
    T* getComponent() const 
    {
        ...
        {
            // 返回 unique_ptr 的裸指针 (肯定是T类型， static_case 其实并不必要，但保留可以使意图更清晰)
