...
void BasicEntityBuilder::buildBase()
{
    ...
    if (object_json_ && object_json_->contains("name"))
    {
        ...
        registry_.emplace<component::NameComponent>(entity_id_, name_id, name);
...
{
    spdlog::trace("构建 Sprite 组件");
    // 如果是自定义形状图像，则不需要 SpriteComponent
    ...
    registry_.emplace<component::SpriteComponent>(entity_id_, tile_info_->sprite_);
}

void BasicEntityBuilder::buildTransform()
{
    spdlog::trace("构建 Transform 组件");
    ...
    if (object_json_)
    {
        ...
        position_ = glm::vec2(position_.x, position_.y - dst_size_.y);   // 图片对象的 position 需要进行调整 (左下角到右上角)
    ...
    // 瓦片层实体，通过 index (Tiled 瓦片层 data 数据的索引) 计算位置
    if (index_ >= 0)
    {
...
{
    spdlog::trace("构建 Animation 组件");
    // 如果存在动画，其性息已经解析并保存在 tile_info_ 中
    if (tile_info_ && tile_info_->animation_)
    {
        ...
        registry_.emplace<component::AnimationComponent>(entity_id_, std::move(animations), animation_id);
    }
}

void BasicEntityBuilder::buildAudio()
{
    spdlog::trace("构建 Audio 组件");
