...
void LevelLoader::loadTileLayer(const nlohmann::json &layer_json)
{
    ...
    size_t index = 0;   // data的数据索引，他决定图块在地图中的位置
    //  -- 每个瓦片都是一个独立的 entity -- 
...
void LevelLoader::loadObjectLayer(const nlohmann::json &layer_json)
{
    ...
    for (const auto &object : objects)
    {
        ...
        else // 如果 gid 存在，则按照图片解析流程
        {
            ...
            if (!tile_info)
            {
                spdlog::warn("对象图层 '{}' 中的对象缺少游戏的 'gid' 或瓦片信息", layer_json.value("name", "Unnamed"));
                continue;
            }

            // 配置生成器，并调用 build，针对图片对象
...
std::optional<engine::component::TileInfo> LevelLoader::getTileInfoByGid(int gid)
{
    ...
    */
    ...
    // --- 考虑多图片的情况 --- 
    if (!is_single_image && !tileset.contains("tiles")) // 没有 tiles 字段的话不符合数据格式要求，直接返回空的瓦片信息
    {
        ...
        if (tile_id == local_id) // 找到对应的瓦片，进行后续操作
        {
            ...
            if (tile_json.contains("animation") && is_single_image && tile_json["animation"].is_array())
            {
                ...
                for (auto& frame : animation)
                {
                    ...
                    auto frame_rect = getTextureRect(tileset, id);  // 根据 id 获取纹理矩形
                    // 源矩形 + 时长，组成一个动画帧
