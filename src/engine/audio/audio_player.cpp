...
{
    Mix_Chunk* chunk = resource_manager_->getSound(sound_id); // 通过 ResouceManager 获取资源
    if (!chunk)
    {
    ...
    if (played_channel == -1)
    {
        spdlog::error("AudioPlayer: 无法播放音效 '{}'，问题: {}", sound_id, SDL_GetError());
    }
    else
    {
...
{
    Mix_Chunk* chunk = resource_manager_->getSound(hashed_path, hashed_path.data()); // 通过 ResouceManager 获取资源
    if (!chunk)
    {
...
bool AudioPlayer::playMusic(entt::id_type music_id, int loops, int fade_in_ms)
{
    ...
    Mix_Music* music = resource_manager_->getMusic(music_id); // 通过 ResouceManager 获取资源
    if (!music)
    {
    ...
    if (!result)
    {
        spdlog::error("AudioPlayer: 无法播放音乐 '{}'，问题: {}", music_id, SDL_GetError());
    }
    else
    {
...
bool AudioPlayer::playMusic(entt::hashed_string hashed_path, int loops, int fade_in_ms)
{
    ...
    Mix_Music* music = resource_manager_->getMusic(hashed_path, hashed_path.data()); // 通过 ResouceManager 获取资源
    if (!music)
    {
    ...
    if (!result)
    {
        spdlog::error("AudioPlayer: 无法播放音乐 id '{}',path: {}: {}", hashed_path.value(), hashed_path.data(), SDL_GetError());
    }
    else
    {
...
{
    // SDL_Mixer 的音量(0-128) 转为 0.0f - 1.0f 的浮点数
