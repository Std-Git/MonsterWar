...
{
    MIX_Audio* audio = resource_manager_->getSound(sound_id); // 通过 ResouceManager 获取资源
    if (!audio)
    {
    ...
    spdlog::trace("AudioPlayer: 播放音效id: '{}'", sound_id);
    return 0;
}
...
int AudioPlayer::playSound(entt::hashed_string hashed_path)
{
    ...
    spdlog::trace("AudioPlayer: 播放音效id: '{}',path: {}", hashed_path.value(), hashed_path.data());
    return 0;
}
...
bool AudioPlayer::playMusic(entt::id_type music_id, int loops, int fade_in_ms)
{
    ...
    MIX_Audio* music = resource_manager_->getMusic(music_id); // 通过 ResouceManager 获取资源
    if (!music)
    {
    ...
    MIX_StopTrack(music_track_, 0);             // 停止之前的音乐
...
bool AudioPlayer::playMusic(entt::hashed_string hashed_path, int loops, int fade_in_ms)
{
    ...
    MIX_Audio* music = resource_manager_->getMusic(hashed_path, hashed_path.data()); // 通过 ResouceManager 获取资源
    if (!music)
    {
