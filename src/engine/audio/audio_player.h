 ...
 * @brief 用于控制音频播放的单例类
 * 
 * @note 提供播放音效的音乐的方法，适用由 ResourceManager 管理的资源
...
private:
    ...
    entt::id_type current_music_id_;                        ///< @brief 当前正在播放的音乐路径，用于避免播放同一音乐
...
public:
     ...
     * @param sound_id 音效ID
     * @param channel 音效播放的特定通道，或 -1 表示第一个可用的通道，默认为 -1
     ...
     * @param fade_in_ms 音乐淡入的时间 (ms) (0 表示不淡入) 默认为 0
    ...
    /**
     * @brief 停止当前正在播放的背景音乐
     * @param fade_out_ms 音乐淡出的时间 (ms) (0 表示不淡出) 默认为 0
     */
     ...
     * @param channel 音效通道，(-1 表示所有通道)，默认为 -1
     */
