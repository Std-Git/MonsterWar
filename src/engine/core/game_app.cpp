...
namespace engine::core
{
    ...
    void GameApp::run()
    {
        if (!init())
        {
            spdlog::error("GameApp 初始化失败，无法进行游戏。");
            return;
        }
    ...
    bool GameApp::init()
    {
        ...
        // 注册回调事件 (回调函数可以无参数，代表不使用事件结构体中的数据)
    ...
    bool GameApp::initSDL()
    {
        ...
        window_ = SDL_CreateWindow(config_->window_title_.c_str(), config_->window_width_, config_->window_height_, config_->window_resizable_ ? SDL_WINDOW_RESIZABLE : 0);
        if (window_ == nullptr)
        {
        ...
        // 设置 VSync (注意：VSync 开启时，驱动程序会尝试将帧率先知道显示器刷新率，有可能覆盖我们手动设置的 target_fps)
    ...
    bool GameApp::initAudioPlayer()
    {
        try
        {
            ...
            // 设置音量
            audio_player_->setMusicVolume(config_->music_volume_); // 设置背景音乐音量为 20%
            audio_player_->setSoundVolume(config_->sound_volume_); // 设置音效音量为 50%
        }
        catch (const std::exception &e)
        {
