...
namespace engine::component
{
    ...
    enum class TileType
    {
        ...
        NORMAL,    ///< @brief 普通瓦片
        SOLID,     ///< @brief 静止可碰撞瓦片
        HAZARD,    ///< @brief 伤害瓦片 (例如：尖刺、火焰等)
        // 未来补充其他类型
    };

    /**
     * @brief 瓦片信息，包含精灵，类型，动画和属性
     * @note 他只是辅助 LevelLoader 解析的临时数据，不会保存在游戏中
     */
    struct TileInfo
    {
        engine::component::Sprite sprite_;                      ///< @brief 精灵
        engine::component::TileType type_;                      ///< @brief 类型
        std::optional<engine::component::Animation> animation_; ///< @brief 动画(支持 Tiled 动画图块)
        std::optional<nlohmann::json> properties_;              ///< @brief 属性 (存放自定义属性，方便 LevelLoader 解析)
    ...
    struct TileLayerComponent
    {
        ...
        std::vector<entt::entity> tiles_;   ///< @brief 瓦片实体列表,每个瓦片代表一个实体，按顺序排列
