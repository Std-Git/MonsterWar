...
/**
 * @brief 表示要绘制的UI图片的数据 (只针对 UI,与游戏中的 Sprite 隔离)
 * 
...
public:
     ...
     * 
     * @param texture_path 纹理资源的文件路径，不能为空
     * @param source_rect 可选的源矩形 (SDL_FRect), 定义要使用的纹理部分，如果为 std::nullopt, 则绘制整个纹理
    ...
    /**
     * @brief 构造一个精灵 (通过纹理 ID 构造)
     * 
     * @param texture_id 纹理资源的标识符，不应为空
     * @param source_rect 可选的源矩形 (SDL_FRect), 定义要使用的纹理部分，如果为 std::nullopt, 则绘制整个纹理
     * @param is_flipped 是否水平翻转
     * @note 用此方法，需确保对应ID的纹理资源已经加载到ResourceManager 中，因此不需要提供纹理路径
     */
    ...
    /**
     * @brief 设置纹理路径同时更新纹理 ID
     * @param texture_path 纹理资源的纹理路径，不应为空
     */
    ...
    void setTextureId(entt::id_type texture_id) { texture_id_ = texture_id; }                          ///< @brief 设置纹理 ID (需确保已载入)
