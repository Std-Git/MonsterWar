...
/**
 * @brief 放置单位系统
 * @note 处理玩家角色准备单位与放置地图单位操作
 *  - 注意不要混淆准备单位(跟随鼠标的待放置单位) 与地图单位(实际放置在地图上的战斗单位)
 */
class PlaceUnitSystem
{
    ...
    /// @brief 目标放置位置实体(即地图上的近战/远程图标), 玩家只能放置在这些位置上
...
private:
    ...
    void onRemoveUnitEvent(const game::defs::RemovePlayerUnitEvent& event); ///< @brief 移除地图上玩家单位事件

    // 输入控制回调函数
    bool onPlaceUnit();        ///< @brief 放置单位 (鼠标左键点击)
    bool onCancelPrepUnit();   ///< @brief 取消放置单位 (鼠标右键点击)
};

}   // namespce game::system