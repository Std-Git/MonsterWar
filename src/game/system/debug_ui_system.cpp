...
void DebugUISystem::renderSelectedUnit()
{
    ...
    auto avaliable_cost = registry_.ctx().get<game::data::GameStats &>().cost_;
...
void DebugUISystem::renderSettingUI()
{
    ...
    // 切换调试工具显示 (勾选结构保存在show_debug_ui_中)
...
{
    // 显示表格，需指定列数（14），标志位使用ImGuiTableFlags_Sortable，表示可排序
    ...
    //  -- 点击标题列，就按照该列排序 -- 
    // 获取排序规格函数 (当sort_specs->SpecsDirty为true时，表示点击了某一列标题，需要重新排序)
    if (ImGuiTableSortSpecs* sort_specs = ImGui::TableGetSortSpecs())
    {
        if (sort_specs->SpecsDirty & !unit_data_list.empty())
        {
            ...
            const bool ascending = (spec.SortDirection == ImGuiSortDirection_Ascending);  // 获取排序方向
            ...
            std::stable_sort(unit_data_list.begin(), unit_data_list.end(),
                             ...
                             {
                                 // 获取职业蓝图 (用于获取属性) pcb = player class blueprint
                                 ...
                                 // delta用于计入比较结果 (-1表示小于，1表示大于，0表示等于)
                                 int delta = 0;
            ...
            // 完成排序后，将SpecsDirty 设置为false, 下轮更新会跳过排序操作(即脏标识模式)
    ...
    for (const auto& unit : unit_data_list)
    {
        ...
        ImGui::Text("%d", static_cast<int>(player_class_blueprint.stats_.range_));
        ...
        if (ImGui::Button(button_text.c_str())) {   // 如果没有PushId, 默认会以Button中显示的参数作为ID,会出现重复ID
            session_data->addPoint(-static_cast<int>(std::round(cost)));
            unit->level_ += 1;
        }
        ImGui::EndDisabled();
        ImGui::PopID(); // 与前面的PushId配对使用, 用于结束ID范围
