...
void BlockSystem::update(entt::registry &registry, entt::dispatcher &dispatcher)
{
    ...
    {
        auto& blocked_by_component = view_blocked_by.get<component::BlockedByComponent>(blocked_by_entity);
        // 如果 BlockedBy 指向的实体无效 (例如死亡), 移除被阻挡者组件，并发送动画 "walk" 事件
        if (!registry.valid(blocked_by_component.entity_))
        { 
            spdlog::info("阻挡者: ID: {}, 无效, 移除 ID: {} 的阻挡者组件", 
                entt::to_integral(blocked_by_component.entity_), 
                entt::to_integral(blocked_by_entity));
    ...
    // -- 判断是否需要添加阻挡者组件
    // 获取所有阻挡者
    auto view_blocker = registry.view<component::BlockerComponent, engine::component::TransformComponent>();
    // 获取所有敌人，使用 entt::exclude 排除“包含指定组件的实体” (已经存在阻挡者组件的敌人不需要在添加)
    auto view_enemy = registry.view<component::EnemyComponent, 
    ...
    for (auto enemy_entity : view_enemy)
    {
        ...
        for (auto blocker_entity : view_blocker)
        {
            ...
            auto& blocker_blocker = view_blocker.get<component::BlockerComponent>(blocker_entity);
                ...
                game::defs::BLOCK_RADIUS * game::defs::BLOCK_RADIUS)
            {
                ...
                // 给敌人添加被阻挡组件
                registry.emplace_or_replace<game::component::BlockedByComponent>(enemy_entity, blocker_entity);
