...
void FollowPathSystem::update(entt::registry &registry, entt::dispatcher &dispatcher, std::unordered_map<int, game::data::WaypointNode> &waypoint_nodes)
{
    ...
    // 筛选条件：速度组件，变换组件，敌人组件，排除"被阻挡的敌人"和"动作锁定敌人"
    ...
    for (auto entity : view)
    {
        ...
        // 如果距离小于阈值，则切换到下一个目标节点 (阈值不要太小，不然敌人速度快的话可能造成震荡)
        if (glm::length(direction) < 5.0f)
        {
            // 如果下一个节点 ID 列表为空，代表达到终点，则发送信号并添加删除标记
            auto size = target_node.next_node_ids_.size();
            if (size == 0)
            {
                spdlog::info("到达终点");
                // 发送信号并添加删除标记
                dispatcher.enqueue<game::defs::EnemyArriveHomeEvent>(); // 具体做什么由回调函数
