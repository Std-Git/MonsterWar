...
void CombatResolveSystem::onAttackEvent(const game::defs::AttackEvent& event)
{
    ...
    float damage = caculateEffectiveDamage(event.damage_, target_stats.def_);
...
// -- 辅助函数 -- 
float CombatResolveSystem::caculateEffectiveDamage(float attacker_atk, float target_def)
{
    // 最终伤害 = 攻击力 - 防御力
