...
{
    // 筛选条件：敌我双方所有攻击型角色 (排除治疗者，治疗这是另外逻辑)
    ...
    for (auto entity : view_has_target)
    {
        ...
        // 检查目标是否在攻击范围内 (检测半径 = 角色攻击范围 + 目标角色半径)
        ...
        if (engine::utils::distanceSquared(transform.position_, target_transform.position_) > range_radius * range_radius)
        {
            // 如果在攻击范围外，则清除目标
            registry.remove<game::component::TargetComponent>(entity);
            spdlog::info("ID: {}, 目标: ID: {}, 超出攻击范围，清除目标", entt::to_integral(entity), entt::to_integral(target.entity_));
            continue;
        }
    }
}
...
void SetTargetSystem::updateNoTargetPlayer(entt::registry& registry)
{
    ...
    auto view_player_no_target = registry.view<engine::component::TransformComponent,
        ...
        game::component::StatsComponent>(entt::exclude<game::component::TargetComponent, game::defs::HealerTag>);
    // 获取所有敌方角色用于检测
    ...
    for (auto player_entity : view_player_no_target)
    {
        ...
        for (auto enemy_entity : view_enemy)
        {
            ...
            {
                // 如果在攻击范围内，则设置目标
                registry.emplace_or_replace<game::component::TargetComponent>(player_entity, enemy_entity);
                spdlog::info("玩家ID: {}, 目标: ID: {}, 设置目标", entt::to_integral(player_entity), entt::to_integral(enemy_entity));
                break;  // 设置一个目标就停止检查
            }
        }
    }
...
void SetTargetSystem::updateNoTargetEnemy(entt::registry& registry)
{
    ...
    for (auto enemy_entity : view_enemy_no_target)
    {
        ...
        for (auto player_entity : view_player)
        {
            ...
            {
                // 如果玩家角色在攻击范围内，则设置目标
                registry.emplace_or_replace<game::component::TargetComponent>(enemy_entity, player_entity);
                spdlog::info("敌人ID: {}, 目标: ID: {}, 设置目标", entt::to_integral(enemy_entity), entt::to_integral(player_entity));
...
void SetTargetSystem::updateHealer(entt::registry& registry)
{
    ...
    for (auto healer_entity : view_healer)
    {
        ...
        // -- 获取血量百分比最低的玩家角色 --
        float lowest_hp_percent = 1.0f;             // 保存最低血量百分比 (初始为 100%)
        entt::entity lowest_hp_player = entt::null; // 保存血量百分比最低的玩家角色 (初始为空)
        // 遍历每一个受伤玩家角色
