...
bool SessionData::loadDefaultData(std::string_view path)
{
    ...
    // map加载完毕后，再将角色数据指针插入到unit_data_list_中
    mapUnitDataList();
    spdlog::info("Session data 已加载：{}", path);
    return true;
}
...
void SessionData::addUnit(std::string_view name, std::string_view class_str, int level, int rarity)
{
    ...
    // 创建角色数据，并插入到 unit_map_ 中
    unit_map_.emplace(name_id, UnitData{name_id, class_id, std::string(name), std::string(class_str), level, rarity});
