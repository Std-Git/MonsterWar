...
constexpr float BLOCK_RADIUS = 40.0f;   ///< @brief 阻挡半径 <next>
constexpr float UNIT_RADIUS = 20.0f;    ///< @brief 角色自身半径 (相当于碰撞盒，用于计算攻击范围)
constexpr float PLACE_RADIUS = 40.0f;   ///< @brief 放置区域半径 (相当于碰撞盒，用于检测鼠标是否在放置区域内)
constexpr float HOVER_RADIUS = 30.0f;   ///< @brief 鼠标悬浮检测半径（相当于碰撞盒，用于检测鼠标是否处在可选中单位上）

constexpr engine::utils::FColor RANGE_COLOR = { ///< @brief 攻击范围显示的颜色
...
enum class PlayerType
{
    ...
    MIXED   ///< @brief 混合型，可以放在任意区域 （暂时没有实现,未来可拓展）
};
